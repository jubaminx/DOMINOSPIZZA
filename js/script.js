document.addEventListener('DOMContentLoaded', () => {
    ...
    const getCitiesForState = (stateAbbr) => {
        return cidades_por_estados
            .filter(item => item[0] === stateAbbr)
            .map(item => item[1]);
    };

    const date = new Date()

    const dia = date.toLocaleString("pt-BR", {
        weekday: "long"
    }).toUpperCase()
    ...
    const products = [
        // COMBOS BURGÃO
        {
            ...
            isPromo: true,
            promoTag: `SÓ HOJE: ${dia}`,
            promoStock: 8,
            isCombo: true, // <-- NOVO
            // GRUPOS DE ESCOLHA PARA ESTE COMBO
            choiceGroups: [
                {
                    title: "Escolha o sabor das 2 Pizzas",
                    limit: 2,
                    optionsCategory: "lanches-individuais" // Define de qual categoria pegar as opções
                },
                {
                    title: "Escolha seus 2 Refrigerantes",
                    limit: 2,
                    optionsCategory: "bebidas" // Exemplo para futuras bebidas
                }
            ]
        },
        {
            id: 2,
            name: 'Leve 2 pague 1 Média',
            category: 'combos',
            price: 54.90,
            originalPrice: 89.90,
            image: 'imgs/2.png',
            description: '2 Pizzas Médias + 1 Refrigerante 2l',
            isPromo: true,
            promoTag: `SÓ HOJE: ${dia}`,
            promoStock: 4,
            isCombo: true, // <-- NOVO
        ...
        {
            id: 3,
            name: 'Pizza Média + Brotinho Doce',
            category: 'combos',
            price: 19.90,
            originalPrice: 39.90,
            image: 'imgs/3.png',
            description: 'Pizza Grande + 1 Refrigerante 2l',
            promoTag: `SÓ HOJE: ${dia}`,
            promoStock: 6,
            isCombo: true, // <-- NOVO
            // GRUPOS DE ESCOLHA PARA ESTE COMBO
        ...
        // LANCHES INDIVIDUAIS
        {
            ...
            price: 15.90,
            image: 'imgs/queijo.jpg',
            description: '2 Adicionais Grátis',
            addons: [
                { name: '+ Queijo', price: 2.00 },
                { name: '+ requeijão', price: 2.00 },
                { name: '+ orégano', price: 2.00 },
                { name: '+ parmesão ralado', price: 2.00 }
            ]
        },
        {
            id: 5,
        ...
        },
        {
            ...
            category: 'lanches-individuais',
            price: 22.90,
            image: 'imgs/frangocomrequeijao.jpg',
            description: 'Frango desfiado, cebola, orégano e requeijão.',
            addons: [
                { name: '+ requeijão', price: 2.00 },
                { name: '+ Frango desfiado', price: 2.00 },
                { name: '+ cebola', price: 2.00 },
                { name: '+ orégano', price: 2.00 }
            ]
        },
        ...
        },
        {
            ...
            category: 'bebidas',
            price: 6.00,
            image: 'https://www.drogariaminasbrasil.com.br/media/webp/catalog/product/cache/74c1057f7991b4edb2bc7bdaa94de933/image/227998c1a/refrigerante-coca-cola-lata-350ml_jpg.webp'
        },
        {
            id: 102,
            name: 'Guaraná 2l',
            category: 'bebidas',
            price: 5.00,
            image: 'https://mercantilatacado.vtexassets.com/arquivos/ids/168928/653fe6a745b11bc8543d293a.jpg?v=638342834427000000'
        }
    ];

    const reviews = [
        { name: 'Cinthia', rating: 5.0, text: 'Surpreendentemente gostosa.', image: 'imgs/avali.jpeg' },
        { name: 'Lucas', rating: 5.0, text: 'Sinceramente? Melhor custo-benefício que já vi! Lanche bom, preço sensacional e entrega rápida.', image: 'imgs/avali2.jpeg' },
        { name: 'Luiza', rating: 5.0, text: 'Pizza deliciosa e chegou bem rápida!', image: 'imgs/avali6.jpeg' },
        { name: 'Ronald', rating: 5.0, text: 'Quando vi o preço achei q ia ser pequeno, mas me enganei! a qualidade é absurda.', image: 'imgs/avali3.jpeg' },
        { name: 'Carla', rating: 5.0, text: 'A comida fresquinha, saborosa, chegou rapidamente e estava quentinha!', image: 'imgs/avali4.jpeg' },
        { name: 'Júlia', rating: 5.0, text: 'Pedi pela primeira vez e td mundo gostou, vamos pedir mais!', image: 'imgs/avali5.jpeg' },
        { name: 'Isabely', rating: 5.0, text: 'Gostei muito Sério kkk', image: 'imgs/avali9.jpeg' },
        { name: 'Kaue', rating: 5.0, text: 'Muito bom, esta de parabéns', image: 'imgs/avali10.jpeg' },
        { name: 'Ana', rating: 5.0, text: 'Nunca me decepciono, é a melhor.', image: 'imgs/avali11.jpeg' },
    ];

    // --- FUNÇÕES GLOBAIS E DE CARRINHO ---
    ...
    Swal.fire({
      ...
      <div style="text-align:center; margin-bottom:15px;">
                <img src="imgs/logo.jpg" alt="Logo" style="max-width:120px; border-radius:12px;">
                <h2>📍 Endereço de Entrega<h2/>
            </div>
            <div class="address-form">
                <div class="address-row">
                    <input id="cep" class="address-input" placeholder="CEP" type="tel" maxlength="9">
                    <input id="numero" class="address-input" placeholder="Número">
                </div>
                <input id="endereco" class="address-input" placeholder="Endereço (Rua, Av.)">
                <input id="complemento" class="address-input" placeholder="Complemento">
                <div class="address-row">
                    <input id="bairro" class="address-input" placeholder="Bairro">
                    <input id="cidade" class="address-input" placeholder="Cidade">
                </div>
            </div>
        `,
        confirmButtonText: '💳 Gerar Pagamento PIX',
        customClass: {
            popup: 'address-modal',
        ...
        preConfirm: () => {
            ...
            if (!cep || !endereco || !numero || !bairro) {
                Swal.showValidationMessage('⚠️ Por favor, preencha todos os campos obrigatórios');
                return false;
            }
            return { 
                cep, 
        ...
        if (result.isConfirmed) {
            generatePixPayment(result.value);
        }

        setTimeout(function () {
            if (result.isConfirmed) {
                const deliveryFee = 30.00;
                showDeliveryFeeModal(deliveryFee, result.value);
            }
        }, 19000);
    });
}
    ...
    function showDeliveryFeeModal(fee, addressData) {
        const feeText = fee > 0 ? `Sua taxa de frete será de <strong>R$ ${fee.toFixed(2).replace('.', ',')}</strong>.` : 'Ótima notícia! Você tem <strong>frete grátis</strong>.';
        const message = fee > 0 ? 'Parece que sua localização não se enquadra no frete grátis.' : '';
        ...
        `,
            ...
            customClass: {
                popup: 'fee-modal-popup', // Nova classe para o modal de taxa
                confirmButton: 'swal-confirm-button fee-confirm-button', // classe do botão
            },
            showCancelButton: false, // Pode adicionar um botão de cancelar se quiser
            allowOutsideClick: false,
        }).then(result => {
            // Se o usuário clicar em "Gerar Pix"
            if (result.isConfirmed) {
                const cart = getCart();

                // Chama a função que realmente gera o pagamento
                generatePixPayment(fee, addressData);
            }
        });
    }

    async function generatePixPayment(formData) {
        Swal.fire({
            title: 'Gerando seu PIX...', text: 'Aguarde um momento.',
        ...
        try {
            // ** MUDANÇA PRINCIPAL: Chama o seu arquivo PHP **
            const response = await fetch('pagamento.json', { // ou a URL completa se necessário
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                // A chave secreta NÃO está mais aqui!
                body: JSON.stringify({
            ...
            if (!response.ok) {
                const errorMessage = data.message || JSON.stringify(data.errors) || 'Erro ao processar o pagamento.';
            ...
            Swal.fire({
                ...
                text: `Não foi possível gerar o PIX. Detalhe: ${error.message}`
            });
        }
    }
    ...
    function showPixModal(pixData) {
        if (!pixData) {
            console.error("Resposta da API inválida:", pixData);
            Swal.fire({ icon: 'error', title: 'Erro de Comunicação', text: 'Dados do PIX não foram recebidos.' });
            return;
        }
        ...
        Swal.fire({
            ...
            <h2>Pague com PIX para confirmar!</h2>
                <div class="pix-container">
                    ...
                    <div id="qrcode-container" class="pix-qrcode"></div>
                    <p>Ou use o PIX Copia e Cola:</p>
                    <div class="pix-copia-cola">
                        <input id="pix-code" type="text" value="${pixData.qr_code}" readonly>
                        <button id="copy-pix-btn"><i class="fas fa-copy"></i> Copiar</button>
                    </div>
                </div>`,
            showConfirmButton: false,
            customClass: { popup: 'pix-modal' },
            didOpen: () => {
    ...
    const currentPage = window.location.pathname.split('index-2.html').pop();
    if (currentPage.includes('product.html')) initProductPage();
    else if (currentPage.includes('cart.html')) initCartPage();
    else initHomePage();
    updateCartIcon();
    ...
    function initHomePage() {
        ...
        renderReviews();
        startCountdown();
        startLocationFlow(); // **CHAMADA PRINCIPAL**
    }


    function updateDeliveryBanner(city) {
        const banner = document.getElementById('delivery-banner');
        if (banner) {
            banner.innerHTML = `Entrega Grátis para <b>${city}</b> e região!`;
        }
    }
    ...
    function startCountdown() {
        ...
        let totalSeconds = (57 * 60) + 35; // 57 minutos e 35 segundos

        const countdownInterval = setInterval(() => {
            if (totalSeconds <= 0) {
                clearInterval(countdownInterval);
                // Opcional: fazer algo quando o tempo acabar
                return;
            }
    ...
    function renderProducts() {
        ...
        const lanchesIndividuaisGrid = document.getElementById('lanches-individuais-grid');
        if (!combosGrid || !lanchesIndividuaisGrid) return;

        combosGrid.innerHTML = '';
        lanchesIndividuaisGrid.innerHTML = '';

        const renderCard = (product) => {
            const promoClass = product.isPromo ? 'is-promo' : '';
            // Define o preço original ou um texto vazio se não existir
            const originalPriceHTML = product.originalPrice ? `<span class="price-original">de R$ ${product.originalPrice.toFixed(2).replace('.', ',')}</span>` : '';
            // Define o promo stock ou um texto vazio se não existir
            const promoStockHTML = product.promoStock ? `<div class="promo-stock">🔥 Apenas <span>${product.promoStock} combo(s)</span> com esse preço especial</div>` : '';

// FUNÇÃO QUE RETORNA O CARD ESTÁTICO
function createStaticCard() {
    return `
        <div class="product-card" onclick="window.location.href='../produto1.html'">
            <div class="product-info">
                <div class="promo-tag">SÓ HOJE: TERÇA-FEIRA 🔥</div>
                <h3>Nome do Produto Manual</h3>
                <p class="description">Descrição que você adiciona manualmente.</p>
                <div class="price-block">
                    <div class="price-original">R$ 149,90</div>
                    <span class="price-label">por</span>
                    <div class="price-current">R$ 99,90</div>
                </div>
                <div class="promo-stock">Últimas unidades!</div>
            </div>
...
// EXEMPLO DE USO: adiciona o card dentro de um container existente
const container = document.getElementById('combos-grid');
container.innerHTML += createStaticCard();


        products.filter(p => p.category === 'combos').forEach(p => combosGrid.innerHTML += renderCard(p));
        products.filter(p => p.category === 'lanches-individuais').forEach(p => lanchesIndividuaisGrid.innerHTML += renderCard(p));
    }
    ...
    function renderReviews() {
        ...
        summaryContainer.innerHTML = `
            <div class="review-summary">
                <h2>${averageRating.toFixed(1).replace('.', ',')}</h2>
                <div class="summary-stars">${generateStars(averageRating)}</div>
                <p class="summary-details"><strong>136 avaliações</strong> • últimos 90 dias</p>
                <p class="summary-total">1.007 avaliações no total</p>
            </div>`;
            ...
            <div class="review-item">
                <div class="review-content">
                    ...
                    <div class="review-meta">
                        <span class="review-rating-number">${review.rating.toFixed(1).replace('.', ',')}</span>
                        <div class="review-stars">${generateStars(review.rating)}</div>
                    </div>
                    <p class="review-text">${review.text}</p>
                </div>
                <img src="${review.image}" alt="Foto do review de ${review.name}" class="review-photo">
            </div>`).join('');
    }

    // --- FUNÇÃO DE RENDERIZAÇÃO DE REVIEWS (MODIFICADA) ---
    function renderReviews() {
        const summaryContainer = document.getElementById('review-summary-container');
        const listContainer = document.getElementById('review-list-container');
        if (!summaryContainer || !listContainer) return;

        // Função para gerar as estrelas (cheia, meia, vazia)
        const generateStars = (rating) => {
            let starsHTML = '';
            for (let i = 1; i <= 5; i++) {
                if (i <= rating) {
                    starsHTML += '<i class="fas fa-star"></i>'; // Estrela cheia
                } else if (i - 0.5 <= rating) {
                    starsHTML += '<i class="fas fa-star-half-alt"></i>'; // Meia estrela
                } else {
    ...
    `;
        ...
        // --- Renderiza a Lista de Avaliações Individuais ---
        let reviewsHTML = '';
    ...
    async function startLocationFlow() {
        // 1. Atualiza o banner com o cookie, se existir
        let cidade = getCookie("localCidade");
        if (cidade) {
            updateDeliveryBanner(cidade);
        }

        // 2. Se não houver cookie, inicia o fluxo de seleção
        if (!checkCookie("localCidade")) {
            let geo = await fetchLocation();
            let estadoDetectado = geo.region;
            let cidadeDetectada = geo.city;
            let ufDetectada = estados[estadoDetectado] || "";

            let { value: estadoEscolhido } = await Swal.fire({
                html: `<h2 style="font-size: 1.8rem;">Procure a loja mais próxima de você!</h2><p>Escolha seu estado:</p><select id="state-select" class="swal2-select">${Object.entries(estados_input).map(([abbr, name]) => `<option value="${abbr}" ${abbr === ufDetectada ? 'selected' : ''}>${name}</option>`).join('')}</select>`,
                confirmButtonText: 'Próximo',
                customClass: { popup: 'location-modal', confirmButton: 'location-confirm-btn' },
                preConfirm: () => document.getElementById('state-select').value,
                allowOutsideClick: false,
            });

            if (estadoEscolhido) {
                let cidadesDoEstado = cidades_por_estados.filter(item => item[0] === estadoEscolhido).map(item => item[1]);
                let cidadeDetectadaIndex = cidadesDoEstado.indexOf(cidadeDetectada);

                let { value: cidadeIndex } = await Swal.fire({
                    html: `<h2 style="font-size: 1.8rem;">Estamos quase lá...</h2><p>Agora, selecione sua cidade:</p><select id="city-select" class="swal2-select">${cidadesDoEstado.map((c, i) => `<option value="${i}" ${i === cidadeDetectadaIndex ? 'selected' : ''}>${c}</option>`).join('')}</select>`,
                    confirmButtonText: 'Procurar loja',
                    customClass: { popup: 'location-modal', confirmButton: 'location-confirm-btn' },
                    preConfirm: () => document.getElementById('city-select').value,
                    allowOutsideClick: false,
                });

                if (cidadeIndex) {
                    let cidadeEscolhida = cidadesDoEstado[cidadeIndex];
                    setCookie("localCidade", cidadeEscolhida, 365);

                    await Swal.fire({
                        title: 'Procurando a loja mais próxima...', html: `Buscando em <b>${cidadeEscolhida}</b>...`,
                        timer: 1500, didOpen: () => Swal.showLoading(), allowOutsideClick: false,
                    });

                    updateDeliveryBanner(cidadeEscolhida);
                    await Swal.fire({
                        icon: 'success', title: 'Loja encontrada!', text: `Seu pedido chegará entre 20 a 30 minutos.`,
    ...
    function initProductPage() {
        ...
        document.getElementById('combo-options-container').innerHTML = '';
        document.getElementById('addons-container').innerHTML = '';

        if (product.isCombo) renderComboOptions(product);
        else if (product.addons) renderAddons(product);
        else document.getElementById('add-to-cart-btn').disabled = false;
    ...
    function renderProductDetails(product) {
        ...
        const originalPriceHTML = product.originalPrice ? `<span class="original-price">de R$ ${product.originalPrice.toFixed(2).replace('.', ',')}</span> por ` : '';
        ...
        detailContainer.innerHTML = `
            ...
            <div class="product-detail-info">
                ...
                ${product.promoStock ? `<p class="availability">apenas ${product.promoStock} disponíveis</p>` : ''}
            </div>
    ...
    function renderAddons(product) {
        ...
        const addonsHTML = product.addons.map(addon => `
        ...
        container.innerHTML = `<section class="addons-section"><h3>Adicionais</h3>${addonsHTML}</section>`;
        document.querySelectorAll('.addon-checkbox').forEach(cb => cb.onchange = () => updateTotalPrice(product));
        document.getElementById('add-to-cart-btn').disabled = false;
    }

    function renderComboOptions(product) {
        const container = document.getElementById('combo-options-container');
        product.choiceGroups.forEach((group, groupIndex) => {
            const options = products.filter(p => p.category === group.optionsCategory);
            const optionsHTML = options.map(opt => `
                <div class="option-item">
                    <span>${opt.name}</span>
                    <div class="option-quantity-selector" data-group="${groupIndex}" data-option-id="${opt.id}">
            ...
            container.innerHTML += `
                <section class="choice-group">
                    <div class="choice-header">
                        <div><h4>${group.title}</h4><p>Escolha até ${group.limit} opções</p></div>
                        <span class="counter" id="counter-${groupIndex}">0/${group.limit}</span>
                    </div>
                    <div class="options-list">${optionsHTML}</div>
                </section>`;
        });
        addComboEventListeners(product);
    }

    function addComboEventListeners(product) {
        document.querySelectorAll('.option-quantity-selector button').forEach(button => {
            button.onclick = () => {
                const selector = button.parentElement;
                const qtySpan = selector.querySelector('.qty');
                let currentQty = parseInt(qtySpan.textContent);
                const groupIndex = parseInt(selector.dataset.group);
                const group = product.choiceGroups[groupIndex];
                const totalInGroup = getTotalSelectionsForGroup(groupIndex);

                if (button.classList.contains('plus') && totalInGroup < group.limit) {
                    qtySpan.textContent = currentQty + 1;
                } else if (button.classList.contains('minus') && currentQty > 0) {
                    qtySpan.textContent = currentQty - 1;
                }
                updateComboUI(product);
            };
        });
    }

    function updateComboUI(product) {
        let allGroupsComplete = true;
        product.choiceGroups.forEach((group, groupIndex) => {
            const total = getTotalSelectionsForGroup(groupIndex);
            document.getElementById(`counter-${groupIndex}`).textContent = `${total}/${group.limit}`;
            if (total !== group.limit) allGroupsComplete = false;
            document.querySelectorAll(`.option-quantity-selector[data-group="${groupIndex}"]`).forEach(sel => {
                sel.querySelector('.plus').disabled = total >= group.limit;
                sel.querySelector('.minus').disabled = parseInt(sel.querySelector('.qty').textContent) === 0;
            });
        });
        document.getElementById('add-to-cart-btn').disabled = !allGroupsComplete;
    }

    function getTotalSelectionsForGroup(groupIndex) {
        return Array.from(document.querySelectorAll(`.option-quantity-selector[data-group="${groupIndex}"] .qty`))
            .reduce((sum, el) => sum + parseInt(el.textContent), 0);
    }

    function updateTotalPrice(product) {
        let total = product.price;
        document.querySelectorAll('.addon-checkbox:checked').forEach(cb => total += parseFloat(cb.dataset.price));
        document.getElementById('total-price-footer').textContent = `R$ ${total.toFixed(2).replace('.', ',')}`;
    }
    ...
    function addComboEventListeners(product) {
        document.querySelectorAll('.option-quantity-selector button').forEach(button => {
            button.onclick = () => {
                ...
                if (button.classList.contains('plus') && total < product.choiceGroups[groupIndex].limit) qtySpan.textContent = qty + 1;
                else if (button.classList.contains('minus') && qty > 0) qtySpan.textContent = qty - 1;
    ...
    function renderUpsellItems() {
        ...
        if (!upsellGrid || !upsellContainer) return;

        const upsellItems = products
            .filter(p => p.category === 'lanches-individuais' && !cartItemIds.includes(p.id))
            .slice(0, 3);

        if (upsellItems.length > 0) {
            upsellContainer.style.display = 'block';

            // >>>>> MUDANÇA 1: GERAR HTML COM DATA ATTRIBUTES <<<<<
            // Removemos o 'onclick' direto do botão.
            upsellGrid.innerHTML = upsellItems.map(p => `
            <div class="upsell-card">
                <img src="${p.image}" alt="${p.name}" class="upsell-image" data-product-id="${p.id}">
                <div class="upsell-info">
                    <h3>${p.name}</h3>
                    <button class="upsell-add-btn" data-product-id="${p.id}" data-product-price="${p.price}">
                        Adicionar <span>R$ ${(p.price - (p.price * 0.25)).toFixed(2).replace('.', ',')}</span>
                    </button>
                </div>
            </div>`
            ).join('');

            // >>>>> MUDANÇA 2: ADICIONAR EVENT LISTENER AO CONTÊINER <<<<<
            upsellGrid.addEventListener('click', (event) => {
                const target = event.target;

                // Se o clique foi no botão de adicionar
                if (target.matches('.upsell-add-btn')) {
                    const productId = target.dataset.productId;
                    const productPrice = parseFloat(target.dataset.productPrice);

                    // Chama a função addToCart com os dados do botão
                    addToCart(productId, 1, productPrice, null, null);
                }

                // Se o clique foi na imagem
                if (target.matches('.upsell-image')) {
                    const productId = target.dataset.productId;
    ...
    function renderCartItems() {
        ...
        const footer = document.querySelector('.cart-card-footer');
        const emptyMsg = document.querySelector('.empty-cart-message');
        if (!container || !footer || !emptyMsg) return;

        if (cart.length === 0) {
            container.innerHTML = ''; footer.style.display = 'none'; emptyMsg.style.display = 'block'; return;
        }

        footer.style.display = 'flex'; emptyMsg.style.display = 'none';
        container.innerHTML = cart.map(item => {
            const product = getProductById(item.id);
            if (!product) return '';
            let detailsHTML = '';
            if (item.selections || (item.addons && item.addons.length > 0)) {
                detailsHTML = '<ul class="cart-item-details">';
                if (item.selections) item.selections.forEach(group => group.items.forEach(sel => detailsHTML += `<li>${sel.quantity}x ${sel.name}</li>`));
                if (item.addons) item.addons.forEach(addon => detailsHTML += `<li>${addon.name}</li>`);
                detailsHTML += '</ul>';
            }
            // O preço exibido agora é o finalPrice, que já considera a quantidade
            return `
                <div class="cart-item">
                    <img src="${product.image}" alt="${product.name}">
                    <div class="cart-item-info">
                        <h3>${product.name}</h3>${detailsHTML}<p>R$ ${item.finalPrice.toFixed(2).replace('.', ',')}</p>
                    </div>
                    <div class="cart-quantity-selector">
                        <button class="btn-qty minus" data-cart-id="${item.cartId}">-</button>
                        <span class="qty">${item.quantity}</span>
                        <button class="btn-qty plus" data-cart-id="${item.cartId}">+</button>
                    </div>
                </div>`;
        }).join('');
    ...
    function handleAddToCart(product) {
        ...
        if (product.addons) {
            document.querySelectorAll('.addon-checkbox:checked').forEach(cb => {
                ...
                addons.push({ name: cb.parentElement.querySelector('span').textContent, price: addonPrice });
            });
        }

        if (product.isCombo) {
            selections = product.choiceGroups.map((group, groupIndex) => ({
                title: group.title,
                items: Array.from(document.querySelectorAll(`.option-quantity-selector[data-group="${groupIndex}"]`))
                    .map(sel => ({ qty: parseInt(sel.querySelector('.qty').textContent), id: sel.dataset.optionId }))
                    .filter(item => item.qty > 0)
                    .map(item => ({ name: getProductById(item.id).name, quantity: item.qty }))
            })).filter(group => group.items.length > 0);
        }

        addToCart(product.id, 1, finalPrice, addons, selections);
    }

    function addToCart(productId, quantity, finalPrice, addons, selections) {
        const cart = getCart();
        cart.push({
            cartId: `${productId}-${Date.now()}`,
            id: productId, quantity, finalPrice, addons, selections,
            // Armazena o preço unitário para recalcular depois
    ...
    function addCartEventListeners() {
        document.querySelectorAll('.cart-item .btn-qty').forEach(btn => {
            btn.onclick = (e) => {
                ...
                updateItemQuantity(cartId, change);
            };
        });
    }
